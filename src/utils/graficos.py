...
class GraficosHelper:
    ...
    def plot_categorica_barras(self, serie: pd.Series, col: str) -> str | None:
        ...
        for bar, v in zip(bars, porcentajes.values, strict=False):
            ax.text(
                v + 0.3,
    ...
    def serie_tiempo(
        ...
        if agrupamiento_sexo and not agrupamiento_entidad:
            ...
            ax.plot(
                ...
                linewidth=1.2,
                alpha=0.8,
                color=self.conf_paleta_sexo["Hombres"],
                label="Hombres",
            )
            ax.plot(
                st.index,
                ...
                linewidth=1.2,
                alpha=0.8,
                color=self.conf_paleta_sexo["Mujeres"],
                label="Mujeres",
            )
            subtitulo = "por sexo"
            ...
            st = (
                ...
                .sum()
                .assign(
                    incrementos_totales=lambda g: g["incrementos_hombres"]
                    + g["incrementos_mujeres"]
                )
                .reset_index()
            )
    ...
    def graficar_pronostico(
        ...
        if metricas:
            ...
            if es_fallback:
                ...
                tokens.append(
                    f"Modelo: Regional ({region})" if region else "Modelo: Regional (respaldo)"
                )
