...
_ORDEN_REGION = ["Metropolitana alta", "Urbana media", "Rural / dispersa", "Sur-Sureste vulnerable"]
...
def _plot_bar(ax, dfx, ycol, catcol, title):
...
def _sort_by(dfp, cat_col, val_col, order_map):
...
def barras_inegi(df: pd.DataFrame):
    ...
    sorted_views = [
        (_sort_by(dfp, "tamano_poblacional_grupo_percentil", "Total", map_tamano),
         "Total", "tamano_poblacional_grupo_percentil", "Población total (color: tamaño percentil)"),
        (_sort_by(dfp, "extension_territorial_percentil", "Superficie_km2", map_extension),
         "Superficie_km2", "extension_territorial_percentil", "Superficie km² (color: extensión percentil)"),
        (_sort_by(dfp, "densidad_poblacional_percentil", "densidad_poblacion", map_densidad),
         "densidad_poblacion", "densidad_poblacional_percentil", "Densidad poblacional (color: densidad percentil)"),
        (_sort_by(dfp, "ratio_h_m_cat", "ratio_h_m", map_ratio),
         "ratio_h_m", "ratio_h_m_cat", "Ratio H/M (color: categoría ratio)"),
        (_sort_by(dfp, "ratio_h_m_cat", "Hombres", map_ratio),
         "Hombres", "ratio_h_m_cat", "Hombres (color: categoría ratio)"),
        (_sort_by(dfp, "ratio_h_m_cat", "Mujeres", map_ratio),
         "Mujeres", "ratio_h_m_cat", "Mujeres (color: categoría ratio)"),
    ]

    has_region = "region_salud_mental" in dfp.columns
    if has_region:
        df_region = _sort_by(dfp, "region_salud_mental", "Total", map_region)
        sorted_views.append((df_region, "Total", "region_salud_mental", "Población total (color: región)"))
        sorted_views.append((df_region, "densidad_poblacion", "region_salud_mental", "Densidad (color: región)"))
...
def boxplots_inegi(df):
    ...
    _boxplot_panel = [
        ("densidad_poblacion", "extension_territorial_percentil", "Densidad por extensión territorial (escala log)", "hab/km²", True),
        ("Total", "tamano_poblacional_grupo_percentil", "Población total por tamaño (escala log)", "Población", True),
        ("Superficie_km2", "extension_territorial_percentil", "Superficie por extensión territorial", "km²", False),
        ("ratio_h_m", "ratio_h_m_cat", "Ratio H/M por categoría", "Ratio H/M", False),
    ]

    if tiene_region:
        _boxplot_panel.extend([
            ("densidad_poblacion", "region_salud_mental", "Densidad por región salud mental (escala log)", "hab/km²", True),
            ("Total", "region_salud_mental", "Población total por región salud mental (escala log)", "Población", True),
        ])
