...
- **Modelado Dengue (per-cohorte, neuro intacto):** grid `dengue` en `prophet.yaml` (multiplicativo, changepoints flexibles) + `_GRID_KEY_MAP`. Los no-neuro entrenan SIN log_transform, SIN holiday COVID (cohort-aware en Prophet, Ensemble y Stacking), con cv_weights uniformes y SIN fallback regional ("si es 0, es 0").
