...
- **SRP**: Maximo 300 lineas por modulo. Excepciones documentadas: `models/deepar/model.py` (complejidad inherente de GluonTS) y los builders de visualizacion/reporte `visualization/{avance5_tables,comparison_bars}.py` (codigo de figuras intensivo en layout, periferico a la logica de modelado; pendiente de particion incremental). Ya particionados (cubrir->partir, con tests smoke/estructurales de red): `comparison_builders.py` -> `comparison_panels` + `comparison_metrics` + `comparison_builders`; `avance5_charts.py` -> `avance5_panels` + `avance5_metric_charts` + `avance5_charts`.
