...
- **Baselines canónicos** (commits limpios): Seasonal Naive `obesidad_benchmark_full_7952e226c10a` @ `cf97301b`; run conjunto 5 motores `obesidad_benchmark_full_fc70f5f15b7b` @ `1e0709fd`; **run vigente 6 motores `obesidad_benchmark_full_6ff688593915` @ `52fc07af`** (mediana sMAPE_all: naive 39.5, median_3y 36.7, mean_3y 32.5, **ETS 30.0 = −24%**, median_5y 28.3, **mean_5y 28.0 = −29%**). Cada motor: 13,312 predicciones base / 23,088 derivadas / 444 métricas / **solo 64 modeladas**; ETS además 256/256 ajustes `primary` convergidos, 0 warnings, `n_train` por fold 366/418/470/522, 19.8 s. Los 5 estacionales dan `metrics.csv` **byte-idéntico** al baseline C3b (forecast/evaluation idénticos salvo la columna `run_id`). Verificado reproducible: mismo `run_id` y los 25 artefactos con el mismo digest en otro `runs_root`. **Sigue sin elegirse ganador.**
