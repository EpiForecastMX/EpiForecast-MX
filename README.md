...
**Production = DeepAR + Prophet + NB-GLM.** All four engines are trained per series (33 geographies × 3 sexes = 99 series), but the productive selector (`scripts/produccion_dengue.py`, by SMAPE on 2026 reality) keeps **only DeepAR and Prophet** (distribution **DeepAR 54 / Prophet 45**; national = Prophet, SMAPE ~20% on 2026 reality). Ensemble and Stacking are excluded because their tree learners do not extrapolate the epidemic dynamic to 52 weeks (a seasonal-envelope guard, `models/forecast_guards.py`, caps them but they are never chosen).
