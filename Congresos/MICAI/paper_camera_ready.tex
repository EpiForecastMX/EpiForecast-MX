...
\rmse\ and carries DeepAR, hence 108 there against 107 here. Because DeepAR's federal-entity margin far exceeds the 5\%
...
W02--W18 (Table~\ref{tab:validation}, Fig.~\ref{fig:validation}); no
2026 observation entered training, tuning or selection
(Sect.~\ref{ssec:eval}).
...
conventional $5\%$ threshold. We therefore read the result as nominal
evidence of lower squared-error loss against DeepAR and Stacking, not
as an established difference, and as no evidence at all against
Prophet. With seventeen weeks the test has little power, so a
...
\midrule
Frozen deployment (baseline) & \multicolumn{2}{c}{---} & 32.7 & ---\\
\bottomrule
\end{tabular}
\end{table}
