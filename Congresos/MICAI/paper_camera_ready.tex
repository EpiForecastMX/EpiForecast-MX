...
to a regional model, and records every decision in a justified log. We instantiate it for weekly
incidence of depressive episodes coded as F32 in the International
Classification of Diseases, 10th Revision (\mbox{ICD-10}), across Mexico's 32
federal entities, using public bulletins of the National Epidemiological
Surveillance System (SINAVE),
...
records about 150{,}000 new cases per year of depressive episode, coded
F32 in the International Classification of Diseases, 10th Revision
(\mbox{ICD-10})~\cite{sinave2026}. Those counts are \emph{retrospective}: without
...
The study uses weekly case counts of \emph{depressive episode}
(\mbox{ICD-10} code \textbf{F32}): a first, single episode of depressed
...
renders the classification acronym in Spanish as \mbox{CIE-10}.
...
is similar but not identical to either ISO~8601 or the
epidemiological-week convention used by the U.S. Centers for Disease
Control and Prevention. We use the calendar as published in the bulletin
and do not re-index against either external standard.} the harmonized panel
...
total (general). We analyze the 32 federal entities and four socio-urban
regions---high-density metropolitan, medium-density urban, vulnerable
South--Southeast, and rural/dispersed---defined from population density
and demographic indicators published by the National Institute of
Statistics and Geography (INEGI). The released files retain the original
Spanish labels. Together with the national series, this yields
$37 \times 3 = 111$ series--stratum combinations.
...
\begin{figure}[t]
\centering
% ALT-TEXT (accessibility / proofing): Two-panel chart: (a) annual reported depression (ICD-10 F32) cases 2014-2026 as bars; (b) weekly national series with the COVID-19 disruption window shaded.
\includegraphics[width=0.80\textwidth]{fig01_temporal_distribution.pdf}
\caption{Depression (\mbox{ICD-10} code F32) temporal distribution. (a) Annual
...
$\log(1{+}y)$ with an ARIMA order selected by the Akaike information
criterion over a compact grid
...
We plan to extend the framework to additional \mbox{ICD-10} codes in the
SINAVE catalog, to report a full-year per-state prospective evaluation,
to add tuned seasonal ARIMA and N-BEATS/N-HiTS baselines, and to
