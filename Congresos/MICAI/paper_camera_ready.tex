...
cross-validated evidence as in the frozen baseline assignment, and
\emph{dynamic}, reselecting on
W03--W11 and scoring on the later, unused weeks W12--W18. Selection runs
...
assignment locked before 2026, and what the dynamic regime must beat.}
\label{tab:ablation}
...
unmoved: the four leave-one-out variants fall within $0.6$ points of the
full pool and none approaches significance (Table~\ref{tab:ablation}).
This holds even when the removal reorganizes the assignment
completely---dropping DeepAR moves 96 of the 99 scored selections onto
other models and moves the median by $0.6$ points. Against a
...
beat Prophet alone here either ($p{=}0.73$), and no leave-one-out
variant improves on it at conventional levels---the best, without DeepAR
($26.4\%$, $p{=}0.07$), is in any case selected after seeing the
outcome. Only removing Prophet separates from the full pool ($29.4\%$,
$p{=}0.02$), one nominal result among five comparisons. The framework's value on this panel is
...
pool improves accuracy here: no leave-one-out variant separates from the
full pool, and a single-model policy matches it. What the same experiment
does support is revision. Against the frozen baseline assignment,
